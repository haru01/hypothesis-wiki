...
現在アクティブなプロジェクトは **agent-platform**（AIエージェントプラットフォームを題材にしたスキル動作確認用ステージング案件）。
...
| agent-platform | AGP | AIエージェントプラットフォーム（作成物支援＋活動ログからインサイト、CLAUDE×LLMwiki×オントロジー）のスキル動作確認用ステージング |
