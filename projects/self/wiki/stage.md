...
この案件は**このキット自体を題材にしたドッグフーディングのサンプル**（接頭辞 `SELF`）。
デスクリサーチ（[[SELF-LEARN-001]]）で企業の仮説検証の状況・課題を洗い出し、そこから課題仮説の種を取って
反証可能な仮説5件（[[SELF-H-001]]〜[[SELF-H-005]]）に起こし、最初の検証計画 [[SELF-TEST-001]] を立てたところ。

**まだ一次インタビューは実施していない**ので、確信度は全件がデスクリサーチ由来の 4（〈二次〉）・`未検証` で止まっている。
次の一手は [[SELF-TEST-001]] を実施し、`/learning` で学び(LEARN)を作って確信度を動かすこと。
