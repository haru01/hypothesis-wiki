...
2. **孤立レコード** — どこからも wikilink で参照されていない仮説／活動／意思決定。ACT が対象仮説にリンクしていない、DEC が根拠 ACT にリンクしていない等。
   - **DEC の判定**: 意思決定レコードは、(a) 根拠 ACT への外向きリンク `[[ACT-NNN]]` を持ち、かつ (b) `wiki/index.md` の「意思決定」節（または stage-transition なら `stage.md` 履歴）から参照されていれば**孤立扱いしない**。仮説からの内向き参照は必須にしない（persevere/pivot/kill/rollback も index 節で被参照されていれば正常）。index 節にも stage.md にも載っていない DEC のみ孤立として報告する。なお **frontmatter `based-on` の欠落は hwlint が `dec-based-on`（warning）で機械検出する**ので、LLM は本文リンク・index 被参照の文脈判断に集中する。
...
9. **架空/デモデータの未明示** — 確信度・「検証済み」の根拠が架空/デモ・シミュレーションデータ（`sources/` 冒頭に「架空」明記があるもの）なのに、仮説レコード・ACT・ビューに「実データ未検証」の注記/フラグが無いもの。実証拠と誤認されうるため要対応として報告する。**確信度が上限（8）を超える行の架空根拠は hwlint が履歴全行を走査して `fictional-cap`（error）で検出する**（最終行だけでなく中間行の架空根拠も取りこぼさない）。LLM は「明示が十分か」の判断に集中する。
10. **関係の型違反・二重表現の欠落**（機械判定） — frontmatter の関係リンクが [ontology.yaml](../../../ontology.yaml) の宣言に反するもの: 接頭辞なし・不在参照・range 種別違反（例: `based-on` が ACT でなく H を指す）・cardinality 違反（単一関係 `derived-from` に複数）・domain/range サブタイプ違反（例: 課題仮説が `addresses` を持つ／`addresses` が課題仮説以外を指す）を **error**（`refs`）で検出する。加えて `must-wikilink` な関係が frontmatter にあるのに本文 wikilink `[[…]]` に無いものを **warning**（`relation-wikilink`）で検出する（二重表現規約: Obsidian グラフに辺を出すため本文にも張る）。さらに H→H 関係（`derived-from`/`leads-to`）の自己参照・循環を **error**（`relation-cycle`）で検出する。
