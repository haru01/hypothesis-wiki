...
- このスキルはレコード（H/TEST/LEARN/DEC）を作らない。器を用意するだけ。中身は `/formulate` 以降で作る。
- 雛形の空ディレクトリ（`hypotheses/` `tests/` `learnings/` `decisions/`）の `.gitkeep` は、最初のレコードを作成した後は削除してよい（任意。`/formulate` `/plan` `/ingest` `/decide` の作成手順が削除する）。
