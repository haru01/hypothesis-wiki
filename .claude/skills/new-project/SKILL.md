...
5. **確認して終了** — 作成したパスと、以後 `/refine` などが `projects/<slug>/` を対象に動くことを伝える。最初の仮説起票は `/refine` へ誘導する。
...
- このスキルはレコード（H/ACT/DEC）を作らない。器を用意するだけ。中身は `/refine` 以降で作る。
- 雛形の空ディレクトリ（`hypotheses/` `activities/` `decisions/`）の `.gitkeep` は、最初のレコードを作成した後は削除してよい（任意。`/refine` `/plan` `/ingest` `/decide` の作成手順が削除する）。
