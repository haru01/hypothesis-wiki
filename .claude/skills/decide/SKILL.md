...
5. **DEC レコードを作る** — `wiki/decisions/` の既存最大+1で `DEC-NNN` を採番、`templates/decision.md` で作成。frontmatter の `id` は**ファイル名と同じ接頭辞つき**にする（例 `id: SELF-DEC-001`）。**frontmatter `based-on` に根拠となった活動を接頭辞つきで列挙し**（must-wikilink）、本文にも根拠 `[[ACT-NNN]]` と影響を受ける `[[H-NNN]]` を必ず wikilink で書く（`based-on` 空の DEC は hwlint が `dec-based-on` で警告する）。**巻き戻しポイント**（誤りのシグナルと戻り先の仮説状態・問い）と **`## 次の一手`**（前を向いて次に何を検証・実行するか＝戦略的現在地。board の「現在地」に最新DECのここが射影される）を必ず明記する。ディレクトリに `.gitkeep` が残っていれば削除してよい。
