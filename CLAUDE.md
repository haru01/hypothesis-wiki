...
| The Schema（設定層） | `CLAUDE.md`・`AGENTS.md`（他エージェント向け入口）・`playbooks/`・`templates/`・`.claude/skills/` | 人間が合意の上で変更する（全プロジェクト共有） |
...
addresses: [H-NNN, ...]              # 省略可（ソリューション仮説）。対応する課題仮説（vp のフィット表になる）
