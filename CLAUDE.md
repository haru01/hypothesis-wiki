...
**wikilinkではなく相対mdリンク**で書く（`[playbooks/cpf.md](../../../playbooks/cpf.md)` の形式。
wikilink `[[playbooks/cpf.md]]` はObsidianで解決せずリンク切れになる）。
...
| 曖昧なアイデアを仮説レコードに精錬する（1問ずつ深掘り） | `/refine` |
...
レコード（H/ACT/DEC）を作るスキル（`/refine` `/plan` `/ingest` `/decide`）は、**壁打ちしながら1問ずつ確認して前に進める**。`/refine` がこのパターンの参照実装。

1. **1問ずつ聞く** — `AskUserQuestion` で一度に1問。複数論点をまとめて聞かない。ユーザーの答えを受けてから次の問いを決める。
2. **曖昧なら深掘りを続ける** — 回答が曖昧・一般論のときは、判断に足るまでさらに1問掘る（「直近の具体例は？」で行動・数値レベルまで降ろす）。
3. **必要コンテキストを引き出してから作る** — そのスキルがレコードを作るのに必須の要素が埋まっていなければ、対話で埋めてから作成に進む（必須要素は各スキルが定義する）。埋まらないうちに空欄や憶測でレコードを確定させない。
4. **確定前に要点を1回確認する** — 対話で固めた要点（何を・どの値で作るか）を提示し、合意を得てから書き込む。全文プレビューは不要、要点1回でよい。
5. **量産しない** — 一度に作りすぎず1件ずつ丁寧に。
