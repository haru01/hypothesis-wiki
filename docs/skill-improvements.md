...
## SI-011: 仮説形成スキルの改名（grill→refine）と対話プロトコルの共通化

- **対象スキル**: `/refine`（旧 `/grill`）`/plan` `/ingest` `/decide`（および `CLAUDE.md` スキーマ層）
- **状態**: 対応済み（`grill`→`refine` リネーム、CLAUDE.md に「対話プロトコル（レコード作成スキル共通）」を新設、plan/ingest/decide に参照＋必須コンテキスト引き出し手順を追記。2026-07-17）
- **気づいた文脈**: スキル改善セッション（2026-07-17）。仮説形成スキルだけが `grill`（問い詰める、の比喩＝手法名）で、他スキル（plan/ingest/decide/view/lint＝目的を表す動詞）と命名がずれて分かりにくかった。また `grill` の「1問ずつ壁打ち」は他のレコード作成スキルにも欲しいが、各スキルに散在していなかった。
- **課題**:
  1. 命名の不一致（手法名 `grill` vs 目的名の他スキル）で、何をするスキルか名前から読めない。
  2. `plan`（手法選択1回）・`ingest`（承認ゲートはあるが1問ずつでない）・`decide`（ほぼ非対話）が、レコード作成に必要なコンテキストを対話で引き出す作りが弱い。空欄・憶測でレコードが確定しうる。
- **改善（反映済みの内容）**:
  1. **リネーム** `grill`→`refine`。規約・案内層（スキル定義・CLAUDE.md・README・テンプレート・案内文）のみ置換し、履歴（log.md・確信度履歴・レコード本文）と生成物（views）・日付つき設計記録の過去 `/grill` 表記は不変ルール3に従いそのまま残置。
  2. **CLAUDE.md に「対話プロトコル（レコード作成スキル共通）」を新設** — 1問ずつ／曖昧なら深掘り継続／必須コンテキストを埋めてから作る／確定前に要点1回確認／量産しない。`refine` を参照実装とする。
  3. **plan/ingest/decide に参照リンクと引き出し手順を追記** — plan=手法提示前に反証条件・制約を1問ずつ、ingest=判定に足りない点を提案前に1問ずつ、decide=DEC作成前にタイプ・移行基準の根拠・巻き戻しポイントを1問ずつ。
- **関連**: `.claude/skills/refine/`（旧grill）、`.claude/skills/{plan,ingest,decide}/SKILL.md`、`CLAUDE.md`「対話プロトコル（レコード作成スキル共通）」。
