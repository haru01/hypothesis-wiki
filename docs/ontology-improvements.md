...
| E-1〜E-4 | 未着手（要設計合意） |
| F-1〜F-3 | 未着手（調査由来・要閾値合意。F1/F2 は lint＋ビューで低リスク） |
...
- **改善案（案）**: importance=focus かつ `hypotheses` 入次数0 のHを「未着手の重点仮説」として warning／
...
- **改善案（案）**: 上記2ギャップを lint warning ＋ `relations` ビューの「フィット表」に空セルとして明示する
