...
## `sources/` の旧ID参照について（意図的に残す）

削除した回顧型 ACT（SELF-ACT-001/005・AIRE-ACT-001/004）を指す `[[wikilink]]` が `sources/` に数件残る
（例 `sources/2026-07-16-desk-research-…`）。`sources/` は**不変層**（rule 3・`guard_sources` が編集をブロック）で、
これらは生データを書いた当時 ACT が実在した歴史的注記である。lint 対象外（`check_wikilinks` は `wiki/` のみ走査）で
実害は Obsidian グラフの切れリンクのみのため、**不変性を優先して修復せず残す**（新規レコード側 LEARN が正しい現在の実体）。
