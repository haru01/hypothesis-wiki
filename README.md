...
- **`/deciding`** — 核心クラスタが移行基準を満たしたので、`SELF-DEC-001` で **CPF→FPF** に移行した。
- **`/building`** — FPF から先取りして「確信度Wiki」の LP（`SELF-TEST-004`、`wiki/prototypes/SELF-TEST-004/index.html`）を
  生成し、提示インタビューにかけた。
...
- **`/chabudai` → `/deciding`（巻き戻し）** — 揺さぶり監査（`SELF-LEARN-005`）で「検証済み」の根拠が架空データ依存だったと露呈し、
