...
├── README.md               # このファイル
├── .claude/skills/         # AgentSkills 6つ（refine/plan/ingest/view/decide/lint・共有）
...
1. `/refine` — アイデアを反証可能な仮説（`<PREFIX>-H-NNN`）にする
...
このリポジトリには、キット自体を題材に実際にスキルを回した実例が入っている（`/refine`→`/plan`→`/ingest`→`/view` の一巡）。
